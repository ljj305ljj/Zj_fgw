...
window.listPage=new Vue({
	...
	methods:{
		...
		getListData :function(para,viewIndex){
			...
			if(item.url == "toread"){ _url += "&handleUserNo=" + eval("("+App.LS.get('userInfo')+")").username; }
			var _type="get";
			// if(item.url == "receival-done"){
			// 	_url += "&filterQueryNotIn=S_suggestionLibrary:1;S_todoReader:*" + eval("("+App.LS.get('userInfo')+")").username + "*"; 
			// }else if(item.url == "dispatch-done"){
            //     _url += "&filterQueryNotIn=S_todoReader:*" + eval("("+App.LS.get('userInfo')+")").username + "*";

			// }
			console.log(_url);			
			ajaxRequst(_url,_type,'','json','').then((result) => {	
				if(item.url == "sldpstodo" || item.url == "sldpsj"){//省领导批示
					$.each(result.list,function(r,res){
						res.businessNo = "receival";
					});
				}			
				if(item.offset==0){
					item.data=result;
