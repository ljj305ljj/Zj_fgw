...
var baseCommon = {
		...
		if(_url=="notGetId"){
			...
			ajaxRequst( _url,"post",'application/json',"json",JSON.stringify(data)).then((json) => {
				if(json[0].id){
					...
					var _url = ZjgyHost + ZjgyUrl["info-tempfile"] + "?list=" + json[0].id + "&os=pc";
					ajaxRequst( _url,"get",'application/json',"json").then((jsondata) => {
						baseCommon.initbasePage(jsondata[0].id);
					});
				}else{
					wispApp.UI.dismissProgressDialog();
	...
	getOpinionList : function(refreshOpin,reportId){
		let _this = this;
		if(App.LS.get("module") == "info"){
			...
			var getopinurl = ZjgyHost + ZjgyUrl["get-opinlist"] + "?docId=" + (reportId?reportId : docInfor.id);
			//var getopinobj = JSON.stringify({docId:docInfor.form.id});
			ajaxRequst( getopinurl,"get",'application/json',"json").then((opinionData) => {
				_this.getOpinionTimes++;
				vFBD["opinionData" +_this. getOpinionTimes] = opinionData;
				if(_this.getOpinionTimes == 1) {baseCommon.getOpinionList(false,docInfor.reportId);}
				if(refreshOpin  && typeof(opinionPage) != "undefined"){opinionPage.opinionList = opinionData;}
				App.LS.set("oplist",JSON.stringify(opinionData));
				if(_this.getOpinionTimes == 2){ 
					let opinion = vFBD.opinionData1.concat(vFBD.opinionData2);
					baseCommon.fullOpinions(opinion);
				}
			});
		}
