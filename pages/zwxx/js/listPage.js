	...
	methods:{
		openDoc : function(obj){			
			...
			if(obj.businessDocId){
				...
				OpenWebView("pages/zwxx/DetailPage.html",{title:"表单页"});
			}else {
				OpenWebView("pages/zwxx/DetailPage.html",{title:"表单页"});
			}			
		},
		...
		getListData :function(para,viewIndex){
			...
			ajaxRequst(_url,_type,'','json','').then((result) => {	
				if(typeof(result) == "string")result = toArr(result);	
				if(item.url == "info-query" || item.url == "info-draft"){
					$.each(result.list,function(r,res){
						res.businessNo = "infodraft";
					});
				}	
