	...
	methods:{
		getDocInfor:function(){	
			...
			if(listInfo.businessDocId){
				var obj = {reportId:listInfo.businessDocId};
			}else{
				var obj = {reportId:listInfo.id};
			}					
			...
			ajaxRequst(_url,'GET','application/x-www-form-urlencoded','json',obj).then(function(json){
				if(json.permission){
					...
					if(moduleId.indexOf("info") > -1){
						//var temp = eval("("+json+")");
						_self.docInfo = json[0];
						App.LS.set("docInfor",JSON.stringify(json[0]));
						//App.LS.set("docId",json[0].attId);
						App.LS.set("docId",json[0].reportId);
						Forms = json[0];
					}else{
						_self.docInfo = json;
...
function submitEnd(){
	...
	var getUrl=ZjgyHost + ZjgyUrl[urlNo]+"?docId="+datainfo.reportId+"&workTodoId="+detailPage.proPermission.workTodoId+"&isConverge=true";
...
function submitCancle(){
	...
	var getUrl=ZjgyHost + ZjgyUrl[urlNo]+'?docId='+datainfo.reportId+'&workTodoId='+listInfo.id+'&processId='+datainfo.flowProcessId;
	ajaxRequst(getUrl,'get','application/json;charset=UTF-8','json').then(function (submitobj) {
			if(submitobj){
				try {wispApp.UI.dismissProgressDialog();} catch(e) {}
				toast("流程操作成功！",3000);				
...
function submitToReadOver(){
	...
	ajaxRequst(getUrl,'post','application/json;charset=UTF-8','json',JSON.stringify([datainfo.reportId])).then(function (submitobj) {
			if(submitobj){
				try {wispApp.UI.dismissProgressDialog();} catch(e) {}
				//toast("阅件成功！",3000);
				return ;
