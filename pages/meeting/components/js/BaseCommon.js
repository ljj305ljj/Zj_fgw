    ...
    getToHandleId : function(){
		...
		switch (thisModule){
			...
			case "meeting_activity":
				_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?moduleId="+thisModule.toUpperCase()+"&type=DEALFORM";
			break;
			case "URGER":
				_url = "URGER";
			break;
		...
		if(_url=="notGetId"){
			...
			if(Forms.urgerTasks){
				var addnum;
				for(let i =0;i<Forms.urgerTasks.length;i++){
					addnum = i;	
					creatHandleTable(tableVal,"",diySet,addnum);
				}
			}else{
				creatHandleTable(tableVal,"",diySet);
			} 
