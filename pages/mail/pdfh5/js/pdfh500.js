    ...
    Pdfh5.prototype = {
		...
		addrenderPdf:function(urlstr){
			...
			pdfjsLib.getDocument(obj).then(function (pdf) {
			    ...
			    self.loading.hide()							
			    self.thePDF[k] = pdf;
				self.thePdfindex[k] = self.totalNum;	//从第几页开始
					...
					(function(k){				
						...
						pdfjsLib.getDocument(tempobj[k]).then(function (pdf) {							
							...
							for (var i=1;i <= pdf.numPages; i++) {
								...
								container.setAttribute('name', 'page=' + page_i);
								container.setAttribute('title', 'Page ' + page_i);
								container.setAttribute('fileindex', ""+ k);		//文件序 标志
								self.viewer[0].appendChild(container);								
								page_i++;
							}
							//alert(page_i)
        ...
        renderCanvas: function (page, viewport, pageNum, num, container, options,curentObj) {
			curentObj={fileUrl:"23144",fileName:"122",className:"12",index:0};
            var self = this;
            var viewport = page.getViewport(options.scale);
            var scale = (self.docWidth / viewport.width).toFixed(2);			
...
/*			if(obj2.src.length<20){
            ...
            return page.render({
                ...
                img.onload = function () {					
					...
					//alert("img loaded"+img.height+"width==" +img.width);				        
                }
		...
		renderMark:function(container,options){
			...
			var defaultSettings = {
				watermark_txt: options.waterMark,
				watermark_txt_len:countnums(options.waterMark),	//水印字个数
				watermark_x: 0,//水印起始位置x轴坐标
				watermark_y: 50,//水印起始位置Y轴坐标
				watermark_rows: 9999,//水印行数
				watermark_cols: 0,//水印列数
				watermark_x_space: 100,//水印x轴间隔
				watermark_y_space: 80,//水印y轴间隔
				watermark_color: '#1f5877',//水印字体颜色
				watermark_alpha: 0.3,//水印透明度
				watermark_fontsize: '20px',//水印字体大小
				watermark_font: '微软雅黑',//水印字体
				watermark_width: 0,//水印宽度120
				watermark_height: 80,//水印长度
				watermark_angle: 35//水印倾斜度数
			};
			//console.log(defaultSettings.watermark_txt_len)
			...
			//获取页面最大长度
			var curwidth=page_width-defaultSettings.watermark_x*2;
			var tempmarkcols=parseInt(curwidth/(10*defaultSettings.watermark_txt_len+defaultSettings.watermark_x_space));	
			var tempwidth=parseInt(curwidth/tempmarkcols);
			//console.log( $("#pageContainer1").height())		
			//var page_height = Math.max(document.body.scrollHeight, $(".pdfViewer").height());
			...
			*/
			...
			for (var i = 0; i < defaultSettings.watermark_rows; i++) {
				...
				//console.log(tempmarkcols);
				//(i%2 ==0?defaultSettings.watermark_cols:defaultSettings.watermark_cols-1)
				var templeft=defaultSettings.watermark_x;
				for(var j=0;j<tempmarkcols;j++){
					if(j==0){
						templeft+=parseInt(tempwidth/2)-defaultSettings.watermark_txt_len*5.5
					}else{						
						templeft+=tempwidth;						
					}
					x=templeft;
...
*url：
	1.文件路径，可以是相对路径"./pdf/nosign.pdf"，也可以支持跨域文件, https://www.gjtool.cn/pdfh5/git.pdf,
	2.文件流
*options:如：{waterMark:"榕基软件"}
		*配置项参数 pdfurl:文件路径，(pdfurl和data二选一)可以是相对路径"./pdf/nosign.pdf"，也可以支持跨域文件, https://www.gjtool.cn/pdfh5/git.pdf,
		*配置项参数 data，文件流形式传入  pdfurl和data二选一
		...
		*配置项参数 loadingBar:是否显示小部件 顶部绿色加载进度条loadingBar 左上角页码显示pageNum 右下角回到顶部按钮backTop  默认显示
		*配置项参数 loadingBar:顶部绿色加载进度条loadingBar 左上角页码显示pageNum 右下角回到顶部按钮backTop  默认显示
		*配置项参数 pageNum:是否显示小部件 左上角页码显示pageNum   默认显示
		*配置项参数 backTop：是否显示小部件 右下角回到顶部按钮backTop 默认显示*
		*配置项参数 scrollEnable:false不允许pdf滚动,true允许pdf滚动  默认允许
		*配置项参数zoomEnable:false不允许pdf手势缩放,true允许pdf手势缩放  默认允许 *
		*配置项参数waterMark:传水印值  默认无水印 *
...
*/
  ...
  function openPdf(Id,url,options){
	$("#app").hide();
	$("#"+Id).show();	
	options.pdfurl=url;
	temp.id=Id;
	...
	var pdfh5 = new Pdfh5('#'+Id, options);
	...
	//pdfh5.addrenderPdf("./pdf/20191223145110898810.pdf");
	/*
	pdfh5.on("ready", function () {
                alert("总页数：" + this.totalNum);
            })
            //监听pdf渲染过程，currentPageDom当前加载的pdf的dom,currentNum当前加载的pdf页数,
            pdfh5.on("render", function (currentNum, time, currentPageDom) {
                alert("当前渲染页：" + currentNum + "，耗时：" + time + "毫秒")
            })
            //监听完成事件，加载失败、渲染成功都会触发
            pdfh5.on("complete", function (status, msg, time) {
                alert("状态：" + status + "，信息：" + msg + "，耗时：" + time + "毫秒，总页数：" + this.totalNum)
            })
            //监听pdf渲染成功
            pdfh5.on("success", function (time) {
                alert("加载完成，耗时" + time + "毫秒");
            })
	*/
  }
