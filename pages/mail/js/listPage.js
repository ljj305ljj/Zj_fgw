...
window.listPage=new Vue({
	...
	methods:{
		goWriteForm (type) {
			App.LS.set("moduleId", this.moduleId)
			App.LS.set("writeForm", type);
			if (type == 'MAIL' || type == 'DEPT_MAIL') {
				App.LS.set("mailSendType", "draft");
				OpenWebView("/pages/mail/writeMail.html", "", "");
			}
		...
		openDoc : function(obj){			
			...
			if(obj.businessDocId){
				...
				OpenWebView("DetailPage.html",{title:"表单页"});
			} else if(this.moduleId == 'moa-mail'){
				if(this.tabActive == '0'){
					App.LS.set('mailType', 'inbox');
					OpenWebView("./pages/mail/mailInforPage.html",{title:"表单页"});
				} else if(this.tabActive == '1') {
					App.LS.set('mailType', 'sent');
					OpenWebView("./pages/mail/mailInforPage.html",{title:"表单页"});
				} else {
					App.LS.set("mailSendType", "fromDraft");
					OpenWebView("./pages/mail/writeMail.html",{title:"表单页"});
				}
				
			}else {
				OpenWebView("./pages/goods/DetailPage.html",{title:"表单页"});
			}			
		},
