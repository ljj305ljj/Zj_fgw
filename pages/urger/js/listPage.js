...
window.listPage=new Vue({
	...
	methods:{
		openDoc : function(obj){	
			...
			if(obj.majorUrger){
				App.LS.set("listInfo",JSON.stringify(obj.majorUrger));
			}else if(obj.listUrger){
				App.LS.set("listInfo",JSON.stringify(obj.listUrger));
			}else{
				App.LS.set("listInfo",JSON.stringify(obj));
			}
			if(obj.businessDocId || (obj.majorUrger && obj.majorUrger.businessDocId)){
		...
		getListData :function(para,viewIndex){
			...
			}else if(item.url == "majorurger-finish" || item.url == "listurger-finish"){
