...
var baseCommon = {
    ...
    getToHandleId : function(divBox){
		...
		switch (thisModule){
			...
			break;
			case "meeting":
				var meetingType = App.LS.get("meetingType");
				_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?moduleId="+thisModule.toUpperCase()+"&type="+meetingType;
			...
			case "meeting_external":
				var meetingType = App.LS.get("meetingType");
				_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?moduleId="+thisModule.toUpperCase();
			break;
			case "URGER":
			case "urgerreport":
				_url = "URGER";
			...
			case "majorurger":
			case "listurger":
				if(App.LS.get("docCate")){
					thisModule="receival"
					_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?docCateNo="+App.LS.get("docCate");	
				}else{
					_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"];	
				}
			...
			break;
			case "meeting_issue":
				_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?moduleId=MEETING&type=ISSUE";
			break;
			case "info":
		...
		if(_url=="notGetId"){
			...
			var tableVal = [];
			let detailTableVal = []; 
			var diySet = {id:"Bldbox",title:"反馈单"};
			if(divBox && divBox == "Bldbox2"){
				diySet = {id:"Bldbox2",title:"反馈单"};
			}
		
			$.each(infos,function(keys,info){
				if(keys=="标题"){
					//diySet.title = "item";
				// }else if(keys == "details"){
				// 	$.each(info,function(ikey,item){
				// 		detailTableVal.push({vname: ikey, vid: item, vtype: "t-span", vcol: "1"});
				// 	})
				}else{
					tableVal.push({vname: keys, vid: info, vtype: "t-span", vcol: "1"});
				}
					
			});
			//creatHandleTable(tableVal,"",diySet);
			//creatHandleTable(tableVal,"");
			
			if(Forms.urgerTaskNodes){
				Forms.urgerTaskNodes.forEach((node,i) => {
					if(node.urgerTaskFeedbacks.length > 0){
						node.urgerTaskFeedbacks[node.urgerTaskFeedbacks.length - 1].nodeTime = new Date(node.nodeTime).Format("yyyy-MM-dd");
						node.urgerTaskFeedbacks[node.urgerTaskFeedbacks.length - 1].signUserName = vFBD.urgerTaskSignUnit().signUserName;				
					}else{
						node.urgerTaskFeedbacks.push({
							nodeTime: new Date(node.nodeTime).Format("yyyy-MM-dd"),
							signUserName: Forms.urgerTaskSignUnits[0].signUserName,
							subject: Forms.urgerTaskSignUnits[0].subject,
							handleObjectName: Forms.urgerTaskSignUnits[0].handleObjectName
						})
					}
					//if(i == 0){
						//let tableVals = [...tableVal,...detailTableVal];
						creatHandleTable(tableVal,"",diySet,i,node.urgerTaskFeedbacks[node.urgerTaskFeedbacks.length - 1]);
					// }else{
					// 	creatHandleTable(detailTableVal,"",diySet,i,node.urgerTaskFeedbacks[node.urgerTaskFeedbacks.length - 1]);
					// }
				});
			}else{
				creatHandleTable(tableVal,"",diySet);
