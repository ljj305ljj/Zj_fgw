...
window.detailPage=new Vue({ 
	...
	created(){	
		...
		try{var moduleId = (listInfo.businessNo || listInfo.S_businessNo ).toLowerCase();}catch(e){var moduleId = (listInfo.S_module).toLowerCase();}
		...
		}else if(listInfo.businessDocId || listInfo.id){
			_self.getDocInfor();
		}else{
			//学习园地
			_self.tabData=[
	...
	methods:{
		getDocInfor:function(){	
			...
			if(listInfo.businessDocId){
				var obj = {id:listInfo.businessDocId};
			}else{
			...
			ajaxRequst(_url,'GET','application/x-www-form-urlencoded','json',obj).then(function(json){
				if(json.permission){
					...
					Forms = json;
					if(_self.docInfo.flowStatus != "finish"){//办结文件不需要获取流程权限
