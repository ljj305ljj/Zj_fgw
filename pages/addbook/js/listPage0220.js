...
window.listPage=new Vue({
	...
	created: function () {		
		...
		dd.ready(function(){
			dd.biz.navigation.setTitle({title : App.LS.get("moduleName")});
		})
		dd.ready(function() {			
	...
	methods:{
		openDoc : function(obj){			
			...
			if(obj.businessDocId){
				...
				OpenWebView("pages/xxgl/DetailPage.html",{title:"表单页"});
			}else {
				OpenWebView("pages/xxgl/DetailPage.html",{title:"表单页"});
			}			
		},
		...
		getListData :function(para,viewIndex){
			...
			if(item.type == "inner"){//组织通讯录
				_url += "&unitNo=" + userinfo.orgNo;
			}*/
			var _type="get";
			console.log(_url);			
			ajaxRequst(_url,_type,'','json','').then((result) => {				
				...
				$.each(item.data.list,function(i,lItem) {
					if($.inArray(lItem.unitName,_self.unitName) == -1){ _self.unitName.push(lItem.unitName); } 
					if(typeof(_self.addbookObj[lItem.unitName])=="undefined")_self.addbookObj[lItem.unitName]=[];
					_self.addbookObj[lItem.unitName].push(lItem);
				})
...
document.addEventListener("visibilitychange", function () {
	if (document.visibilityState == "visible") {//从表单页面切回
		if (getCookieValue("callback") && getCookieValue("callback") != ""){
			eval("(" + getCookieValue("callback") + ")");
		} 
	}
});