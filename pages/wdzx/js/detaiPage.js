...
window.detailPage=new Vue({ 
	...
	created(){	
		...
		// 		//{name:"领导批示",pageId:"Leader",loadUrl:"./components/LeaderPage.html",initFlag:"false",datas:[]},
	...
	methods:{
		getDocInfor:function(){	
			...
			if(listInfo.moduleId == "bbs" || listInfo.moduleId == "partyBuilding"){//通知公告、机关党建
		...
		getAllFiles:function(){
			...
			ajaxRequst(_url,'get','application/json;charset=UTF-8','json',data).then(function(json){
				...
				if(json.length==0){//没有文件，去掉文件展示页面
					...
					_self.tabData.push({name:"文件预览",pageId:"FileRead",loadUrl:"components/FileReadPage.html",initFlag:"false",datas:[]});
					_self.tabnum =_self.tabData.length; 
				}
				_self.$nextTick(function(){
					$.each(_self.tabData,function(i,item){
