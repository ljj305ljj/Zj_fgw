...
window.detailPage=new Vue({ 
	...
	created(){	
		...
		} else if(moduleId == "goods" || moduleId == "supplies"|| moduleId == "transfer"  ){
			$.each(moudleInfo,function(i,Item){
				if(Item.badge== moduleId.toUpperCase()){								
					_self.tabData=[].concat(Item.formTab);
					_self.getDocInfor();
				}
			})
		}else if(listInfo.businessDocId){
			_self.getDocInfor();
		}else{
			//学习园地
			_self.tabData=[
	...
	methods:{
		getCommonOpinion:function(){
			...
			var docinfor = toArr(App.LS.get('docInfor'));
			var personOpinion=[
				{"class":"0","name":"公用意见","id":"COMMON",datalist:[],tourl:ZjgyHost + ZjgyUrl["getcommonOpinion"]+"?opinionCategory=COMMON&docCate=" + docinfor.docCate},
				{"class":"1","name":"个人意见","id":"PERSON",datalist:[],tourl:ZjgyHost + ZjgyUrl["getpersonOpinion"]+"?opinionCategory=PERSON&userNo=" + userinfo.userNo},
			];
			$.each(personOpinion, function(i,item) {
				var _url = item.tourl;
				ajaxRequst( _url,"get",'application/json',"json",{}).then((json) => {
					personOpinion[i].datalist=json;
					if(i==personOpinion.length-1){
		...
		getFlowOpinion(){
			this.getCommonOpinion()
			let _self = this;
			if (this.proPermission.stateBusiness && this.proPermission.stateBusiness.businessOpinion) {
				var businessOpinion = this.proPermission.stateBusiness.businessOpinion[0]
				_self.opinionInfo = {
					businessNo: this.proPermission.businessNo,
					opinionNo: businessOpinion.opinionNo,
					optionName: businessOpinion.opinionName
				}
			}
		...
		toEvents: function(name,thisObj){
			...
			//closePage();
			if(name=="jsEnd" || name=="jsOver" || name=="jsFreeReturn" || name=="jsCancle" || name=="startProcess" || name=="saveForm" || name=="jsAutoSend" || name=="jsRevoke" || name=="jsForceFinish"){
				eval("("+name+"())");
			}else{
				App.LS.set("flowType",name);
				//OpenWebView("/FlowBoxPage.html");
				if (name == "writeOpin") {
					OpenWebView("/FlowBoxPage.html", [this.opinionInfo]);
				}  else {
					OpenWebView("/FlowBoxPage.html",{title:"操作页"});
				}
			}
		},
		initBtnData : function(){					
			...
			if($.inArray("forcefinish",baseAction)>-1 ){
				//办结
				btns.push({btnName:"强制办结",iconCss:"banjie",toEvent:"jsForceFinish"});
			}
		...
		getProcessPermission:function(moduleId){//流程权限获取
			...
			ajaxRequst(_url,'get','application/json;charset=UTF-8','json',obj).then(function(json){
				...
				App.LS.set("proPermission",JSON.stringify(json));//流程操作
				_self.getAllFiles();
				_self.initBtnData();
			})
		},
...
function submitRevoke(){
	...
	var process = eval("("+App.LS.get("proPermission")+")");

	var urlNo="Tzf-"+App.LS.get("module");
	var module = App.LS.get('module');
	var getUrl=ZjgyHost + ZjgyUrl[urlNo]
	ajaxRequst(getUrl,'post','application/json;charset=UTF-8','json',JSON.stringify([process.processId])).then(function (submitobj) {
			if(submitobj){
...
function jsForceFinish(){
	...
	App.UI('dialog', {
			...
			if(backflag==='OK'){
				...
				// }
				ajaxRequst(getUrl,'post','application/json;charset=UTF-8','json',JSON.stringify([datainfo.flowProcessId])).then(function (submitobj) {
					if(submitobj){
						try {wispApp.UI.dismissProgressDialog();} catch(e) {}
						toast("办结成功！",3000);				
						closePage("refreshList()");
