...
var baseCommon = {
    ...
    getToHandleId : function(){
		...
		switch (thisModule){
			...
			break;
			case "goods":
				//_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?name="+encodeURI('基本信息阅办单')+"&os=pc";
				_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?name="+encodeURI('办公用品意见阅办单')+"&os=pc";
			break;
			case "supplies":
				_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?name="+encodeURI('办公耗材意见阅办单')+"&os=pc";
			break;
			case "transfer":
				_url = ZjgyHost + ZjgyUrl[thisModule+"-bld"]+"?name="+encodeURI('基本信息阅办单')+"&os=pc";
			break;
		...
		if(_url=="notGetId"){
			...
			ajaxRequst( _url,"get",'application/json',"json").then((json) => {
				if(json[0].id){
					...
					if(thisModule == "goods" || thisModule == "supplies"){
