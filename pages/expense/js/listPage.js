	...
	methods:{
		openDoc : function(obj){			
			...
			if(obj.businessDocId){
				...
				OpenWebView("pages/expense/DetailPage.html",{title:"表单页"});
			}else {
				OpenWebView("pages/expense/DetailPage.html",{title:"表单页"});
			}			
		},
		...
		getListData :function(para,viewIndex){
			...
			if(item.url == "car_apply-bj"){//办结
				_url += "&passReaderStr="+eval("("+App.LS.get('userInfo')+")").username;
			}
			var _type="get";
			console.log(_url);			
			ajaxRequst(_url,_type,'','json','').then((result) => {
				if(item.url == "car_apply-bj" ){//车辆办结
					$.each(result.list,function(r,res){
						res.businessNo = "CAR_APPLY";
					});
				}
				if(item.offset==0){
					item.data=result;
		...
		isShowdraftBtn(){//列表上是否显示起草按钮
			var item = this.tabData.view[this.tabActive];
			if(item.type == 'draft' || item.type == 'todo'){
				return true;
			}else{ 
				return false; 
			}
		},
		GetFlowList(){//起草-获取起草流程
			App.LS.set('Urlpara','');
			App.LS.remove("viewType");
			App.LS.set("viewType","newDraft");
			listInfo = {
				businessNo:"TRAVEL_EXPENSE",
			}
