...
window.listPage=new Vue({
	...
	created: function () {		
		...
		var _self=this;		
		_self.moduleId= 'moa-gzdcx';
		_self.getModuleInfor('moa-gzdcx');
		$("title")[0].innerText= '工资单列表';	
		dd.ready(function(){
	...
	methods:{
		openDoc : function(obj){			
			...
			OpenWebView("./pages/finance/DetailPage.html",{title:"表单页"});			
		},
		...
		getListData :function(para,viewIndex){
			...
			console.log(ZjgyUrl['gzdcx-info'])			
			var _url=ZjgyHost + ZjgyUrl['gzdcx-info']+"?cardType="+JSON.parse(App.LS.get('financeInfo')).cardType+"&year="+JSON.parse(App.LS.get('financeInfo')).year+"&userNo="+JSON.parse(App.LS.get('userInfo')).username
			var _type="get";
			console.log(_url);			
